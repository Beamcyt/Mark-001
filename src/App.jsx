...
export default function App() {
  ...
  // Load users + sync currentUser realtime
  useEffect(() => {
    if (!currentUser) return;
    const u = onSnapshot(collection(db,"users"), snap => {
      const list = snap.docs.map(d=>({id:d.id,...d.data()}));
      setUsers(list);
      // sync currentUser ให้ทันสมัยตลอดเวลา
  ...
  const summary = {};
  // เริ่มจาก user ทุกคนที่ไม่ใช่ pending (ให้แสดงแม้ยังไม่มีใบลา)
  const summaryUsers = isAdmin
    ? users.filter(u => u.role !== "pending")
    : users.filter(u => u.id === currentUser?.id && u.role !== "pending");
  summaryUsers.forEach(u => { summary[u.name] = { vacation:0, personal:0, sick:0, other:0 }; });
  ...
  async function updateStatus(id, status) {
    try {
      ...
      if (leave?.createdBy) {
        ...
        await fsSetDoc(doc(db,"notifications",nid), {
...
<div style="display:flex; justify-content:space-between; text-align:left;"><img src="${logoUrl}" style="max-height:90px;max-width:210px;object-fit:contain"/><h1 style="margin:5px; text-align:right;">ใบลา</h1></div>
...
<div class="sig">
  ...
  <div>ผู้ขอลา</div>
  <div style="margin-top:6px;font-size:12px;color:#555">วันที่ &nbsp;_____/_____/_____</div>
</div>
<div class="sig">
  <div class="sig-line"></div>
  <div>พนักงานที่ควบคุม</div>
  <div style="margin-top:6px;font-size:12px;color:#555">วันที่ &nbsp;_____/_____/_____</div>
  <div style="font-size:12px;color:#555;margin-top:2px">&nbsp;</div>
</div>
<div class="sig">
  <div class="sig-line"></div>
  <div>ผู้อนุมัติ</div>
...
<tr><th>ลาพักผ่อน</th><th>ลากิจ</th><th>ลาป่วย</th><th>ลาพิเศษ 2</th><th>อื่นๆ</th></tr>
<tr><td>ลามาแล้ว</td><td></td><td></td><td></td><td></td><td></td><td></td></tr>
<tr><td>ลาครั้งนี้</td><td></td><td></td><td></td><td></td><td></td><td></td></tr>
<tr><td>รวม</td><td></td><td></td><td></td><td></td><td></td><td></td></tr>
                        ...
                        return (
                          <tr key={name} style={{ borderBottom:"1px solid #f1f5f9" }}>
                            ...
                            {["vacation","personal","sick","other"].map(t=>{
                              const used = s[t]||0;
                              const qval = parseFloat((users.find(x=>x.name===name)?.leaveQuota||{})[t])||0;
                              const over = qval>0 && used>qval;
                              return (
                                <td key={t} style={{ padding:"8px 12px",minWidth:80 }}>
                                  <span style={{ fontWeight:600,color:over?"#ef4444":"#0f172a" }}>{used>0?used.toFixed(1):""}</span>
    ...
    <div style={{ position:"fixed",inset:0,zIndex:600,background:"rgba(15,23,42,.5)",
      ...
      <div style={{ background:"#fff",borderRadius:16,width:"100%",maxWidth:440,
        ...
        <div style={{ marginBottom:16 }}>
          ...
          <input value={manager} onChange={e=>setManager(e.target.value)} style={IS} placeholder="เช่น นายณัฐพงศ์ รัตนไชย"/>
        </div>
        <div style={{ marginBottom:16 }}>
          <label style={LB}>🏢 สำนัก/ฝ่าย</label>
          <input value={department} onChange={e=>setDepartment(e.target.value)} style={IS} placeholder="เช่น โครงการและการจัดการบริการ"/>
        </div>
        <div style={{ marginBottom:16 }}>
          <label style={LB}>🏷 พนักงานระดับ</label>
          <input value={empLevel} onChange={e=>setEmpLevel(e.target.value)} style={IS} placeholder="เช่น 2"/>
        </div>
        <div style={{ marginBottom:16 }}>
          <label style={LB}>💼 ตำแหน่ง</label>
          <input value={position} onChange={e=>setPosition(e.target.value)} style={IS} placeholder="เช่น เจ้าหน้าที่บริการลูกค้า"/>
        </div>
        <div style={{ marginBottom:20 }}>
          <label style={LB}>🪪 เลขบัตรพนักงาน</label>
          <input value={empId} onChange={e=>setEmpId(e.target.value)} style={IS} placeholder="เช่น MT67033"/>
        </div>
